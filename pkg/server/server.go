...
const (
	resourceName          string = "tweakzx/mobile"
	defaultMobileLocation string = "/etc/mobile"
	mobileSocket          string = "mobile.sock"
...
// mobileServer 是一个 device plugin server
...
// NewmobileServer 实例化 mobileServer
func NewmobileServer() *mobileServer {
...
func (s *mobileServer) Run() error {
	...
	err = syscall.Unlink(DevicePluginPath + mobileSocket)
	if err != nil && !os.IsNotExist(err) {
		return err
	}

	l, err := net.Listen("unix", DevicePluginPath+mobileSocket)
	if err != nil {
		return err
	}
	...
	conn, err := s.dial(mobileSocket, 5*time.Second)
	if err != nil {
		return err
	}
	conn.Close()
...
func (s *mobileServer) RegisterToKubelet() error {
	...
	req := &pluginapi.RegisterRequest{
		...
		Endpoint:     path.Base(DevicePluginPath + mobileSocket),
...
// Manager
func (s *mobileServer) GetDevicePluginOptions(ctx context.Context, e *pluginapi.Empty) (*pluginapi.DevicePluginOptions, error) {
...
func (s *mobileServer) ListAndWatch(e *pluginapi.Empty, srv pluginapi.DevicePlugin_ListAndWatchServer) error {
...
func (s *mobileServer) Allocate(ctx context.Context, reqs *pluginapi.AllocateRequest) (*pluginapi.AllocateResponse, error) {
	...
	for _, req := range reqs.ContainerRequests {
		...
		resp := pluginapi.ContainerAllocateResponse{
			Envs: map[string]string{
				"mobile_DEVICES": strings.Join(req.DevicesIDs, ","),
			},
		}
...
func (s *mobileServer) PreStartContainer(ctx context.Context, req *pluginapi.PreStartContainerRequest) (*pluginapi.PreStartContainerResponse, error) {
	log.Infoln("PreStartContainer called")
	return &pluginapi.PreStartContainerResponse{}, nil
}

// listDevice 从节点上发现设备
func (s *mobileServer) listDevice() error {
	dir, err := ioutil.ReadDir(defaultMobileLocation)
	if err != nil {
		return err
	}
...
func (s *mobileServer) watchDevice() error {
	...
	err = w.Add(defaultMobileLocation)
	if err != nil {
...
func (s *mobileServer) dial(unixSocketPath string, timeout time.Duration) (*grpc.ClientConn, error) {
